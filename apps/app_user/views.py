...
from app_user.serializers import RegSerializer,LogSerializer,ReceiveFileSerializer
from app_user.models import ReceiveFileModel,UserModel
...
#查看当前登录用户
class Who_am_I_ViewSet(G,M.ListModelMixin):
...
class ReceiveViewSet(ModelViewSet):
    queryset = ReceiveFileModel.objects.all().order_by('id')
